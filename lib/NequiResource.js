  ...
  _responseHandler: function(req, callback) {
    ...
    return function(res) {
      ...
      res.on('end', function() {
        ...
        // NOTE: Nequi responds with lowercase header names/keys.
        ...
        try {
          ...
          if (response.error) {
            ...
            if (res.statusCode === 401) {
              err = new Error.NequiAuthenticationError(response.error);
            } else if (res.statusCode === 403) {
              err = new Error.NequiPermissionError(response.error);
            } else if (res.statusCode === 429) {
              err = new Error.NequiRateLimitError(response.error);
            } else {
              err = Error.NequiError.generate(response.error);
            }
          ...
          return callback.call(
            ...
            new Error.NequiAPIError({
              message: 'Invalid JSON received from the Nequi API',
              response: response,
              exception: e,
      ...
      var d = {
        'RequestMessage': {
          'RequestHeader': {
            ...
            'MessageID': nanoid(alphabet, 10),
            'ClientID': self._nequi.getApiField('clientId') || 'nequi-node',
            'Destination': {
              'ServiceRegion': data.service.region || 'C001',
              'ServiceVersion': data.service.version || '1.0.0'
            }
          },
          'RequestBody': {
            'any': data.body
