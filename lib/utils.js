...
var OPTIONS_KEYS = ['accessKey', 'secretKey', 'apiKey', 'channel', 'service'];
  ...
  getDataFromArgs: function(args) {
    ...
    if (optionKeysInArgs.length > 0 && optionKeysInArgs.length !== argKeys.length) {
      console.warn( // eslint-disable-line no-console
        'Nequi: Options found in arguments (' + optionKeysInArgs.join(', ') + '). Did you mean to pass an options ' +
        'object?'
      );
    }
  ...
  */
  getOptionsFromArgs: function(args) {
    ...
    if (args.length > 0) {
      ...
      if (utils.isOptionsHash(arg)) {
        var params = args.pop();
        if (params.accessKey) {
          opts.auth.accessKey = params.accessKey;
        }
        if (params.secretKey) {
          opts.auth.secretKey = params.secretKey;
        }
        if (params.apiKey) {
          opts.headers['x-api-key'] = params.apiKey;
          opts.auth.apiKey = params.apiKey;
        }
        if (params.channel) {
